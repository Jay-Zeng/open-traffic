...
object RouteTTViterbi {
  ...
  def list(feed: String, nid: Int, dates: Seq[LocalDate] = Seq.empty[LocalDate], net_type: String): Seq[FileIndex] = {
    val dic = dates.toSet
    val dir_name = "%s/%s/viterbi_rtts_nid%d_%s/".format(Files.dataDir(), feed, nid, net_type)
  ...
  def fileName(feed: String, nid: Int, date: LocalDate, net_type: String) = {
    "%s/%s/viterbi_rtts_nid%d_%s/%s.json.gz".format(Files.dataDir(), feed, nid, net_type, Dates.dateStr(date))
  }
